...
const ModalConfigEdit = ({ isOpen, onClose, onDataUpdate, data }) => {
  const [state, setState] = useState(data || DEFAULT_STATE);
  const [config, setConfig] = useState([]);
  let configType = data?.configType || "";
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (state?.configType && state?.config) {
        let response;
        response = await dataSource.shared?.updateSystemConfig(state);
        showToast(response);
        setState(DEFAULT_STATE);
        onDataUpdate();
        onClose();
  ...
  return (
    <B2Modal
      title={"Edit System Config : " + configType}
      className={"b2form"}
      isCentre={true}
      isOpen={isOpen}
