...
const ModalCategoryCreate = ({ isOpen, onClose, onDataUpdate, data }) => {
  ...
  const handleSubmit = async () => {
    try {
      let response;
      response = await dataSource.shared?.createCategory(state);
      showToast(response);
      setState(DEFAULT_STATE);
      onDataUpdate();
      onClose();
  ...
  return (
    <B2Modal title={"ADD PLATFORM"} className={"b2form"} isCentre={true} isOpen={isOpen}
