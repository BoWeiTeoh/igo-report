...
const ModalProfile = ({ isOpen, onClose, onDataUpdate, data }) => {
  ...
  const handleSubmit = async () => {
    try {
      let response;
      response = await dataSource.shared?.resetPassword(state);
      showToast(response);
      setState(DEFAULT_STATE);
      onClose();
    } catch (e) {
  ...
  return (
    <B2Modal title={"Profile Details"} className={"b2form"} isCentre={true} isOpen={isOpen}
      ...
      <div className="modal-form-content">
        <div className="modal-form-content-1">
          ...
          <div>
            ...
            <B2Input onChange={handleInputChange} type={"password"} name={"oldPassword"} value={state?.oldPassword}
                     placeholder={" "} />
          </div>
          <div>
      ...
      {/* <div>
        ...
        <B2Row>
          <B2Col md={8}>
            <B2Input onChange={handleInputChange} type={"password"} name={"oldPassword"}
                     value={state?.oldPassword} placeholder={" "}
            />
          </B2Col>
        </B2Row>
        <label>New Password</label>
